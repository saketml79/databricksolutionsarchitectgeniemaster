...
export default function App() {
  ...
  return <main className="architect-shell">
    ...
    <section className="review-packages"><div className="review-heading"><div><p className="eyebrow">REVIEW PACKAGES</p><h2>{showArchived ? 'Archived proposals.' : 'Proposals waiting for a decision.'}</h2></div><span>{packages.length} loaded</span></div><button className="archive-toggle" onClick={() => setShowArchived((current) => !current)}>{showArchived ? 'Back to pending proposals' : 'View archived proposals'}</button>{loadError || decisionError ? <p className="load-error">{loadError || decisionError}</p> : <div className="package-list">{packages.map((reviewPackage) => <article className="package-row" key={reviewPackage.proposal_id}><div><strong>{reviewPackage.title}</strong><p>{reviewPackage.proposal_id}</p></div><div><span className="status-pill">{reviewPackage.status}</span><p>{reviewPackage.package_status ?? 'ARTIFACTS_PENDING'}</p></div><div className="artifact-links">{reviewPackage.pdf_path && <a href={`/api/artifacts/${encodeURIComponent(reviewPackage.proposal_id)}/pdf`} target="_blank" rel="noreferrer">Open PDF</a>}{reviewPackage.status === 'PENDING_APPROVAL' && !reviewPackage.svg_path?.includes('_option_1.svg') && <div className="decision-controls"><button disabled={decisionInFlight === reviewPackage.proposal_id} onClick={() => recordDecision(reviewPackage, 'APPROVED')}>Approve</button><button className="reject" disabled={decisionInFlight === reviewPackage.proposal_id} onClick={() => recordDecision(reviewPackage, 'REJECTED')}>Reject</button></div>}</div>{reviewPackage.svg_path && (reviewPackage.svg_path.includes('_option_1.svg') ? <OptionDiagrams proposalId={reviewPackage.proposal_id} selectedOptionId={reviewPackage.selected_option_id} evidenceRefreshToken={evidenceRefreshToken} onDecision={reviewPackage.status === 'PENDING_APPROVAL' ? (optionId, decision) => recordOptionDecision(reviewPackage, optionId, decision) : undefined} decisionInFlight={decisionInFlight} /> : <img className="package-preview" src={`/api/artifacts/${encodeURIComponent(reviewPackage.proposal_id)}/svg`} alt={`${reviewPackage.title} architecture diagram`} />)}</article>)}</div>}</section>
...
type ReviewPackage = { proposal_id: string; title: string; status: string; package_status?: string; svg_path?: string; png_path?: string; pdf_path?: string; selected_option_id?: string };
