...
async function queryAsRequestingUser(request: Request, statement: string) {
  ...
  if (!accessToken) throw new Error('Your Databricks OAuth access token is unavailable for this request. Refresh the App and try again.');
  const userWorkspace = new WorkspaceClient({ host: workspaceHost, authType: 'pat', token: accessToken });
  const result = await userWorkspace.statementExecution.executeStatement({ warehouse_id: warehouseId, statement, disposition: 'INLINE', format: 'JSON_ARRAY', wait_timeout: '50s', on_wait_timeout: 'CANCEL' });
